...
Овде дајемо неке примере из рачунарства који су довољно мали али илустративни да се покакжу неки концепти tla
спецификације.
...
Особина исправности је \textit{светло је увек у неком од два дозвољена стања}, тј. \texttt{TypeInvariant} дефиниција важи у сваком достижном стању.
...
припада неком скупу могућих вредности. Могли смо све ово записати као $x \in \{p, q\}$, идеја програма би имала исти смисао.
\end{primer}
...
Обе варијанте су семантички еквивалентне; булова је компактнија, а стринг варијанта је читљивија када стања имају смислена имена. Приметимо
да смо овде користили нови оператор \texttt{\~} који нам представља логичку негацију.
\end{primer}
...
Два процеса се такмиче за приступ критичној секцији, делу кода који сме да извршава само један процес у времену.
...
Разложимо сваки сегмент спецификације.

\textbf{Параметар и почетно стање.}
\texttt{CONSTANT N} уводи број процеса као параметар чија вредност се задаје у конфиг фајлу.
\texttt{ASSUME} гарантује да TLC не прихвата безначајне вредности попут \texttt{N = 0}.
\texttt{Procs == 1..N} је скуп идентификатора процеса, нпр.\ за \texttt{N = 2} то је $\{1, 2\}$.
Стање система је \textbf{функција} \texttt{state} дефинисана над скупом \texttt{Procs} израз
\texttt{[p \textbackslash in Procs |-> "idle"]} прави функцију која сваком процесу додељује \texttt{"idle"}.

\textbf{Акција \texttt{TryEnter(p)}.}
Процес прелази из \texttt{idle} у \texttt{trying}  пријављује намеру да уђе у критичну секцију.
Нема додатних предуслова: сваки \texttt{idle} процес може у сваком тренутку покушати улазак.
\texttt{[state EXCEPT ![p] = "trying"]} прави нову функцију идентичну \texttt{state} осим за процес \texttt{p}.

\textbf{Акција \texttt{Enter(p)}.}
Процес прелази из \texttt{trying} у \texttt{critical}.
Кључни предуслов је \texttt{\textbackslash A q \textbackslash in Procs : q \# p => state[q] \# "critical"},
процес \texttt{p} може ући само ако \emph{ниједан} други процес тренутно није у критичној секцији.
Ово је сама суштина узајамног искључења директно изражена у спецификацији.
Важно је напоменути да ова спецификација \textbf{не описује механизам} (браве, семафоре) итд. она само
описује \emph{шта} смеме да се деси, а не \emph{како} то постићи у имплементацији.

\textbf{Акција \texttt{Exit(p)}.}
Процес напушта критичну секцију и враћа се у \texttt{idle}.
Нема услова осим да процес мора бити у \texttt{critical}, систем не моделује шта се дешава унутар критичне секције.

\textbf{Акција \texttt{Next}.}
\texttt{\textbackslash E p \textbackslash in Procs} означава да у сваком кораку TLC бира неки процес \texttt{p}
и извршава једну од три акције за њега. Ово моделује \textbf{произвољно преплитање}, TLC ће
истражити све могуће редоследе извршавања.

\textbf{Особине.}
\texttt{TypeInvariant} проверава да \texttt{state[p]} увек остаје у скупу \texttt{States} за све процесе.
\texttt{MutualExclusion} је главна safety особина: за свака два \emph{различита} процеса \texttt{p} и \texttt{q}
(\texttt{p \# q}), не може се десити да су оба у \texttt{critical} истовремено.
Симбол \texttt{\textasciitilde} означава логичку негацију, цела формула је импликација: ако су \texttt{p} и \texttt{q} различити, онда њихова стања не могу оба бити \texttt{"critical"}.
\end{primer}
