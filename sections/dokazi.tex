...
\begin{itemize}
    \item \textbf{Zenon} је аутоматски доказивач за логику prvog реда,
    \item \textbf{Isabelle/TLA+} је интерактивни доказивач теорема,
    \item \textbf{SMT солвери} нпр.\ Z3 за аритметичке леме.
\end{itemize}
...
\texttt{<1>n. QED} & завршни корак нивоа --- обједињује претходне кораке \\
...
особина која важи за \emph{све} вредности бројача, без обзира на величину.
...
Ово је образац \textbf{индуктивног инваријанта} --- најчешћа техника доказивања safety особина у TLA+:
\begin{enumerate}
    \item Докажи да инваријанта важи у почетном стању (\textit{initiation}).
    \item Докажи да ако инваријанта важи пре прелаза, важи и после (\textit{consecution}).
...
у скупу дозвољених вредности --- особина коју смо раније само проверавали TLC-ом.
...
Доказ следи исти образац индуктивног инваријанта, али са мање корака јер систем има само једну акцију.
...
\begin{napomena}
Model checking (TLC) и доказивање теорема (TLAPS) се \textbf{допуњују}: TLC је бржи и аутоматски
проналази грешке у конкретним моделима, dok TLAPS даје потпун доказ за све могуће вредности параметара.
Уобичајен радни ток је прво верификовати спецификацију са TLC-ом, а затим формално доказати кључне особине са TLAPS-ом.
\end{napomena}
